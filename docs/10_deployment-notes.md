...
## 8. batch.yml の Claude Code ヘッドレス実行が未トラストで失敗していた(2026-07-23 修正済み)

`.claude/settings.local.json`(`permissions.allow` を持つ)がリポジトリにコミットされているため、
CIのチェックアウトにも展開される。GitHub Actionsの新規ワークスペースは未トラスト状態で、
Claude Code はこの状態だと `permissions.allow` を無視するだけでなく `claude -p` が exit code 1 で失敗する
(`this workspace has not been trusted` の警告を stderr に出力)。digestは記事単位で失敗を許容するため
partial扱いで進むが、curateは全滅して `curate: failed` になっていた。

対応: `batch.yml` に「Trust workspace for Claude Code」ステップを追加し、バッチ実行前に
`~/.claude.json` の `projects[$GITHUB_WORKSPACE].hasTrustDialogAccepted` を `true` にする。

**教訓**: ヘッドレスの `claude -p` をCIで回すときは、コミット済みの `.claude/settings.local.json` の
有無にかかわらず、実行前にワークスペースをトラスト済みにしておくこと。
